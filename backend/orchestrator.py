...
from claude_runner import run_analyzer
from demo_findings import DEMO_FINDINGS
from models import AnalyzerStatus, AnalyzerType, Finding
from report_builder import build_report
from settings import get_int_env
from store import store

_BUNDLED_DEMO_PATH = (Path(__file__).resolve().parent.parent / "sample_project").resolve()

# Staggered delays (seconds) for each analyzer in demo mode so the UI shows
# realistic incremental progress. Total wall-clock time stays under 20 s.
_DEMO_DELAYS: dict[AnalyzerType, float] = {
...
_DEMO_FINDINGS_BY_TYPE: dict[AnalyzerType, list[Finding]] = {}
for _f in DEMO_FINDINGS:
    _DEMO_FINDINGS_BY_TYPE.setdefault(_f.category, []).append(_f)
...
async def run_scan(scan_id: str, project_path: str) -> None:
    ...
    try:
        ...
        # Build scorecard and finalize
        scan = store.get(scan_id)
        if scan:
            scan.findings = all_findings
            scan.scorecard = build_report(all_findings)
...
def is_bundled_demo(project_path: str) -> bool:
    """Return True when project_path points to the bundled sample project."""
    try:
        return Path(project_path).resolve() == _BUNDLED_DEMO_PATH
    except OSError:
        return False


async def run_demo_scan(scan_id: str, project_path: str) -> None:
    """Fast demo mode: replay pre-generated findings with staggered delays.
    ...
    try:
        tasks = [
            asyncio.create_task(
                _run_demo_analyzer(scan_id, analyzer_type)
            )
...
async def _run_demo_analyzer(
    scan_id: str,
    analyzer_type: AnalyzerType,
) -> list[Finding]:
    """Simulate a single demo analyzer with a pre-set delay."""
    store.update_analyzer_status(scan_id, analyzer_type, AnalyzerStatus.RUNNING)
    await asyncio.sleep(_DEMO_DELAYS.get(analyzer_type, 5.0))
    findings = _DEMO_FINDINGS_BY_TYPE.get(analyzer_type, [])
